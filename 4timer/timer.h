...
private:
    ...
    std::function<void()>m_cb;   //触发超时的回调函数
    TimerManager *m_manager = nullptr;   //管理timer的的管理器
...
public:
    ...
    //添加条件timer
    std::shared_ptr<Timer> addTimer(uint64_t ms,std::function<void()>cb,std::weak_ptr<void>weak_cond,bool recurring = false);
...
protected:
    ...
    //添加timer
    virtual void addTimer(std::shared_ptr<Timer> timer);
