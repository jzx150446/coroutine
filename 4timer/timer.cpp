...
bool Timer::cancel()
{
    std::unique_lock<std::shared_timed_mutex> write_lock(m_manager->m_mutex);
    if(m_cb==nullptr){
        return false;
    }else{
        m_cb = nullptr;
...
m_ms(ms),m_recurring(recurring),m_cb(cb),m_manager(manager)
{
...
std::shared_ptr<Timer>TimerManager:: addTimer(uint64_t ms,std::function<void()>cb,std::weak_ptr<void>weak_cond,bool recurring)
{
...
{
    std::unique_lock<std::shared_timed_mutex>read_lock(m_mutex);
    m_tickled = false;
    if(m_timers.empty()){
        return ~0ull;
    }
...
void TimerManager::listExpiredCb(std::vector<std::function<void()>>& cbs)
{
    ...
    std::unique_lock<std::shared_timed_mutex> write_lock(m_mutex); 
